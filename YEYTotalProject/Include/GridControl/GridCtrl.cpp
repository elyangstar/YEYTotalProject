...
//  feel are necessary, and sometimes dufe to existing bugs. Where possible I
...
void CGridCtrl::EnsureVisible(int nRow, int nCol)
{
	...
	if(nRow<VisibleCells.GetMinRow() || nRow>VisibleCells.GetMaxRow()) //태현190818 이동하려는 범위가 현재 표시 위치에서 벗어나면 스크롤 위치 강제 이동하고, 해당 위치 데이터 재 계산 한다.
	{
		...
		VisibleCells = GetVisibleNonFixedCellRange(NULL, TRUE);
